...
class TSAForce:
    ...
    def __init__(self, force, is_universe=False):
        ...
        Args:
            force: probably list
                List of force points(?) in priority order(?)
            is_universe: bool
                WTF is this
        """
    ...
    def __repr__(self):
        if self.is_universe: return 'TSAForce({}, is_universe=True)'.format(repr(self.force))
        return 'TSAForce({})'.format(repr(self.force))
...
class TSAResult:
    ...
    def __repr__(self):
        return 'TSAResult({}, desc={}, is_and={}, cases={})'.format(
                    repr(self.force),
                    repr(self.desc),
                    repr(self.is_and),
                    repr(self.cases)
    ...
    def _output(self, case, indent, force, do_case):
        ...
        for c in range(len(self.cases)):
            ...
            if one_case:
                ...
                if curc != 0:
                    res.append(pad + '    ----------')
                res.append(self.cases[c]._output(case + ('' if case == '' else '.') + '{}'.format(curc+1), indent+1, force, True))
                curc += 1
...
class TSA:
    def __init__(self, p, q, depth):
        ...
        Args:
            ...
            q: <permuta.MeshPatt>
                The mesh pattern we want to obtain and show coincidence with p.
            depth: <numbers.Integral>
                The maximum depth of the reccurrence tree.
        """
    ...
    def init_dfs(self, mp, putin, force, xyval, seen):
        ...
        for d in DIRS:
            ...
            possforce |= res.force
            cases.append(TSAResult(res.force, desc='Choose the {} point in {}'.format(STR_ADJ[d], putin), cases=[res]))
    ...
    def dfs(self, mp, force, xyval, seen):
        """Main procedure that handles the recursing TSA algorithm.
        """
        # print seen
        xval, yval = xyval
        ...
        for occ in choose(len(xval), self.k):
            ...
            if Q_CHECK and self.q.shading <= sub.shading:
            ...
            desc1 += '\nmp = {}'.format(boxes)
            desc1 += '\nadds = {}'.format(adds)
            desc1 += '\nsubxval = {}'.format(subxval)
            desc1 += '\nsubyval = {}'.format(subyval)
            assert len(boxes) > 0
            # if len(boxes) == 0:
            #     print repr(self.p)
            #     print repr(self.q)
...
# [1 2 3] PATTERNS
    ...
    # run = tsa4(MeshPatt(Perm([0]), [(1,1)]), (0,1), 5)

    #C1
    run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(0,1),(1,0),(2,0),(2,2),(3,0),(3,2),(3,3)]), (2,1), 3)
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(0,1),(1,0),(2,0),(2,2),(3,0),(3,2),(3,3)]), (2,1), (1, 3),  3)

    #C2
    # mp1 = MeshPatt(Perm([0,1,2]), [(0,1),(1,0),(2,0),(2,2),(3,0),(3,2),(3,3)])
    # mp2 = mp1.shade((2,1))
    # run = TSA(mp1, mp2, 3).run()
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,0),(2,0),(2,2),(3,0),(3,2),(3,3)]), (2,1), 3)

    #C5
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(2,0),(2,2),(3,0),(3,2),(3,3)]), (2,1), 3)

    # ---------------------------------------------------------------------------- #

    #C8
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,3),(1,0),(1,1),(1,2),(2,0),(2,2),(3,0)]), (2,3), 3)

    #C9
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(0,3),(1,0),(1,1),(1,2),(2,0),(2,2),(3,0)]), (2,3), 5)

    #C14
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,0),(1,1),(2,3),(3,0),(3,1),(3,2),(3,2),(3,3)]), (2,1), 3)
    ...
    # C17
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,1),(1,3),(2,1),(2,2),(2,3),(3,0),(3,3)]), (1,0), 3)

    # C18
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,1),(1,3),(2,1),(2,2),(2,3),(3,0)]), (1,0), 3)

    # ---------------------------------------------------------------------------- #

    # C19
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,0),(1,3),(2,1),(3,0)]), (3,3), 4)

    # C20
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,3),(2,1),(3,0)]), (3,3), 5)

    # ---------------------------------------------------------------------------- #

    # C21
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(0,1),(1,1),(2,1),(2,2),(3,0)]), (3,2), 3)

    # C22
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,0),(1,1),(2,1),(2,2),(3,0)]), (3,2), 5)

    # C23
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,1),(2,1),(2,2),(3,0)]), (3,2), 3)

    # ---------------------------------------------------------------------------- #

    # C24
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,1),(2,1),(2,2),(2,3),(3,0),(3,3)]), (1,0), 3)

    # ---------------------------------------------------------------------------- #

    # C25
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,0),(1,1),(1,2),(2,3),(3,0)]), (3,3), 5)

    # C26
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,1),(1,2),(2,3),(3,0)]), (3,3), 6)

    # C27
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,2),(2,3),(3,0)]), (3,3), 6)

    # C28
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,2),(2,3),(3,0)]), (3,3), 4)

    # C29
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,1),(1,2),(2,3),(3,0)]), (3,3), 4)

    # ---------------------------------------------------------------------------- #

    # C30 This is a chain
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,0),(1,1),(2,3),(3,0)]), (2,1), 3)
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,0),(1,1),(2,1),(2,3),(3,0)]), (3,3), 4)

    # ---------------------------------------------------------------------------- #

    # C31
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,1),(2,1),(2,2),(2,3),(3,0)]), (1,0), 4)

    # ---------------------------------------------------------------------------- #

    # C32
    #run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,2),(2,2),(3,0),(3,3)]), (0,0), 5)

    # C33
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,2),(2,2),(3,0),(3,2),(3,3)]), (0,0), 6)

    # C34
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,2),(3,0),(3,3)]), (0,0), 6)

    # C35
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,2),(2,2),(3,0)]), (0,0), 4)

    # C36
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,2),(3,0)]), (0,0), 4)
    ...
    # C37
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,1),(2,1),(2,2),(3,0)]), (3,2), 6)

    # ---------------------------------------------------------------------------- #

    # C38 This is a chain
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(2,2),(3,0),(3,2),(3,3)]), (2,1), 6)
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(2,1),(2,2),(3,0),(3,2),(3,3)]), (0,0), 6)


    # C85
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1, 2), (3, 3), (2, 3), (2, 2), (0, 3), (1, 1)]), (0,1), 5)
    ...
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(0,1),(1,0),(2,0),(2,2),(3,0),(3,2),(3,3)]), (2,1), 3)
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(0,1),(1,0),(2,0),(2,2),(3,0),(3,2),(3,3)]), (1,1))
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(1,0),(1,3),(2,1),(3,0)]), (3,3))
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,1),(1,0),(2,0),(2,2),(3,0),(3,2),(3,3)]), (2,1))
    # run = tsa4(MeshPatt(Perm([0,1,2]), [(0,0),(1,0),(1,1),(2,3),(3,0),(3,1)]), (2,1))
...
# [1 3 2] PATTERNS
    ...
    # C6_3
    # run = tsa4(MeshPatt(Perm([0,2,1]), [(0, 1), (1, 3), (2, 3), (0, 2)]), (1,1), 2)

    # C32
    # run = tsa4(MeshPatt(Perm([0,2,1]), [(0,1),(1,0),(3,2)]), (1,1), 5)

    # C34
    #run = tsa4(MeshPatt(Perm([0,2,1]), [(0,1),(1,0),(1,2),(1,3),(2,2),(3,2)]), (1,1), 100000)

    # C35
    #run = tsa4(MeshPatt(Perm([0,2,1]), [(0,1),(1,0),(2,2)]), (1,1), 2)

    # C36
    # run = tsa4(MeshPatt(Perm([0,2,1]), [(0,1),(1,0)]), (1,1), 2)

    # C39
    # run = tsa4(MeshPatt(Perm([0,2,1]), [(0,1),(1,0),(2,2),(2,3)]), (1,1), 5)

    # C39
    # run = tsa4(MeshPatt(Perm([0,2,1]), [(0,1),(1,0),(2,2),(2,3)]), (1,1), 10)

    # C69
    # run = tsa4(MeshPatt(Perm([0,2,1]), [(3, 0), (0, 3), (2, 3), (1, 0), (2, 2)]), (3,2), 4)

    # C85
    # run = tsa4(MeshPatt(Perm([0,2,1]), [(0, 1), (1, 2), (0, 0), (2, 1), (0, 3)]), (1, 3), 6)


    #run = tsa4(MeshPatt(Perm([0,2,1]), []),(0,0), 100)
