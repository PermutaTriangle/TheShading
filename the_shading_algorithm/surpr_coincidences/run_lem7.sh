...
for p in {"01","012","021"}; do
    for i in {0..32}; do
        python classify.py results/surprising_coincidence_"$p"_SL_Lemma7_"$i".txt -lemma7 > results/surprising_coincidence_"$p"_SL_Lemma7_$((i + 1)).txt

        diff results/surprising_coincidence_"$p"_SL_Lemma7_"$i".txt results/surprising_coincidence_"$p"_SL_Lemma7_$((i + 1)).txt 2> /dev/null
