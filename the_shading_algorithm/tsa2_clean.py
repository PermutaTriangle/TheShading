...
class TSA:
    ...
    def dfs(self, mp, force, xyval, seen):
        ...
        for occ in choose(len(xval), self.k):
            ...
            if len(adds) == 0:
                desc2 = desc1 + '\nThis is another instance of p where the point (%d,%d) is more to the %s, which means that this branch leads to a contradiction.' % (force[0]+1, self.p.perm[force[0]], STR_ADJ2[force[1]])
